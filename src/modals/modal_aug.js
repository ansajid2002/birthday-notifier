...
const ModalAug = () => {
    const {peopData,isModalOpen,closeModalAug} = useGlobalContext()

    const singleInfo = peopData.filter((d) => {
        return (d.bMonth === "August")
    })
    return (
    <div className={isModalOpen.aug ? "modal-overlay show-modal" : "modal-overlay"}>
        ...
        <button onClick={closeModalAug} className="close-btn"><FaTimes/></button>
        
    </div>
  )
}

export default ModalAug